    ...
    : IRequestHandler<RegisterCommand, Result<AuthResponse>>
{
    ...
    {
        logger.LogInformation("Handling RegisterCommand for phone number: {PhoneNumber} and email: {Email}", request.PhoneNumber, request.Email);
        
        var phoneExists = await dbContext.Users.AnyAsync(u => u.PhoneNumber.Value == request.PhoneNumber, cancellationToken);
        if (phoneExists)
        {
            logger.LogInformation("Phone number {PhoneNumber} already exists", request.PhoneNumber);
            
            return Result.Failure<AuthResponse>(Error.Conflict("User.PhoneExists",
        ...
        {
            logger.LogError("Failed to register user with phone number: {PhoneNumber} and email: {Email}. Reason: {Reason}",
                request.PhoneNumber, request.Email, userResult.Error.Message);
        ...
        logger.LogInformation("Successfully registered user with ID {UserId}. Access token generated", user.Id);
